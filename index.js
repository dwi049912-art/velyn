...
const {
  Client,
  GatewayIntentBits,const express = require("express");
const app = express();
...
client.on("guildMemberAdd", async (member) => {
  try {
    ...
    <svg width="900" height="350" xmlns="http://www.w3.org/2000/svg">
      <style>
        .title { font: bold 64px Arial, sans-serif; fill: #FFD700; }
        .name { font: bold 30px Arial, sans-serif; fill: #FFFFFF; }
      </style>
      <text x="450" y="255" text-anchor="middle" class="title">WELCOME</text>
      <text x="450" y="300" text-anchor="middle" class="name">${safeName}</text>
    </svg>`;

    const svgData = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(textSvg)}`;
    const textLayer = await loadImage(svgData);
